...
import { CalendarPlus, CaretDown, Clock, CurrencyDollar } from '$assets/icons'

interface InputProps {
  variant: 'base' | 'phone' | 'weight' | 'price' | 'status' | 'logo' | 'date' | 'time'
  name: string
  label?: string
...
<template>
  ...
  <div v-if="variant === 'date'" class="FormInput Phone w-full">
    ...
    <div class="Wrap relative">
      ...
      <div
        class="Number absolute top-1/2 left-3 z-10 flex h-8 w-8 -translate-y-1/2 items-center justify-center gap-3 rounded-md pt-2"
      >
